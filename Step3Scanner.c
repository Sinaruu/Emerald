...
extern emerald_intg transitionTable[NUM_STATES][CHAR_CLASSES];
...
emerald_intg startScanner(BufferPointer psc_buf) {
	...
	for (emerald_intg i=0; i<NUM_TOKENS;i++)
...
Token tokenizer(emerald_void) {
	...
	emerald_char c;			/* input symbol */
	emerald_intg state = 0;	/* initial state of the FSM */
	emerald_intg lexStart;	/* start offset of a lexeme in the input char buffer (array) */
	emerald_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	emerald_intg lexLength;	/* token length */
	emerald_intg i;			/* counter */
	///emerald_char newc;		// new char

	/* Starting lexeme */
	emerald_strg lexeme;	/* lexeme (to check the function) */
	lexeme = (emerald_strg)malloc(VID_LEN * sizeof(emerald_char));
	if (!lexeme)
		return currentToken;
	lexeme[0] = EOS_CHR;
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((emerald_intg)lexLength + 2, READER_DEFAULT_FACTOR);
			if (!lexemeBuffer) {
...
emerald_intg nextState(emerald_intg state, emerald_char c) {
	emerald_intg col;
	emerald_intg next;
	col = nextClass(c);
...
emerald_intg nextClass(emerald_char c) {
	emerald_intg val = -1;
	switch (c) {
	case UND_CHR:
		val = 2;
...
Token funcCMT(emerald_strg lexeme) {
	...
	emerald_intg i = 0, len = (emerald_intg)strlen(lexeme);
...
Token funcIL(emerald_strg lexeme) {
	...
	if (lexeme[0] != EOS_CHR && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			scData.scanHistogram[currentToken.code]++;
			currentToken.attribute.intValue = (emerald_intg)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcID(emerald_strg lexeme) {
	...
	emerald_char lastch = lexeme[length - 1];
	emerald_intg isID = EMERALD_TRUE;
	switch (lastch) {
		case AMP_CHR:
...
Token funcKEY(emerald_strg lexeme) {
	Token currentToken = { 0 };
	emerald_intg kwindex = -1, j = 0;
	emerald_intg len = (emerald_intg)strlen(lexeme);
...
emerald_void printToken(Token t) {
	extern emerald_strg keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case MNID_T:
		...
		break;
	case STR_T:
		printf("STR_T\t\t%d\t ", (emerald_intg)t.attribute.codeType);
		printf("%s\n", readerGetContent(stringLiteralTable, (emerald_intg)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
